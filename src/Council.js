...
import { jsonCopy, subjectFromCouncilUrl,
...
class Kingdom extends Component {
  ...
  addSubject(e) {
    e.preventDefault()
    const subject = subjectFromCouncilUrl(this.state.value)
      ...
      <div className="Kingdom">
        <div className="row">
          ...
          <div className="col-lg-12">
            <h2><Link to="/animals">MyPicture</Link></h2>
        </div>
        <div className="col-lg-12">
            <h2><Link to="/territories">MyBanner</Link></h2>
        </div>
        <div className="col-lg-12">
            <h2><Link to="/others">Other Councils</Link></h2>
        </div>
        </div>
        <div className="row ruler">
          <div className="col-lg-12">
            <h2>Council Member {username}</h2>
            {rulerAnimal ?
              <p><a href={`${EXPLORER_URL}/name/${username}`} target="_blank" rel="noopener noreferrer">{username}</a> {rulerAnimal.name}</p>
              :
              <p>{username}</p>
            }
            <p>
            {mine ? <Link className="btn btn-primary" to="/me" role="button">Edit my council</Link>
            : <a
              className='btn btn-primary'
              href={`${window.location.origin}/council/${currentUsername}?add=${app}/council/${username}`}
            >Add member to my council
            </a>}
            </p>
                  ...
                  >
                    <form onSubmit={this.addSubject} className="input-group">
                      <input
                        ...
                        required
                        placeholder="https://example.com/council/member.id"
                      />
